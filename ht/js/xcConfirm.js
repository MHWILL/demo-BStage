...
(function($){
	...
	window.wxc.xcConfirm = function(popHtml, type, options) {
		...
		if(config.w.length!=0&&config.h.length!=0){
			$popBox.width(config.w);
			$popBox.height(config.h);
			$popBox.css('margin-left',-(config.w/2)+'px');
			$popBox.css('margin-top',-(config.h/2)+'px');
		}else{
			$popBox.width('570px');
			$popBox.height('260px');
			$popBox.css('margin-left','-285px');
			$popBox.css('margin-top','-130px');
		}
